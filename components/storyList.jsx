...
module.exports = React.createClass({
  render: function() {
    let {stories, title, id, deselectCountry, feedClicked} = this.props;
    ...
    return <div id={id} className="story-list">
      <h3>{title} <small className="clickable" onClick={deselectCountry}>Full Country List</small></h3>
      <div className="row">
        <div className='col-sm-12 col-md-6'>
          {positives.length > 0 ?
            positives.map(function(story) {
              return <Story key={story.link} feedClicked={feedClicked} {...story} />
            })
